    ...
    <v-card class="pa-4">
      ...
      <v-data-table-server v-model:options="options" :items="items" :items-length="totalItems" :headers="headers"
        ...
        <template #[`item.action`]="{ item }">
          <v-btn :href="`/devices/${item.id}`" variant="text">
            <template v-slot:prepend>
              <v-icon color="info" icon="mdi-content-duplicate" size="small" />
            </template>
            Clone
          </v-btn>
          <v-btn :href="`/devices/${item.id}/edit`" variant="text">
            <template v-slot:prepend>
...
export default {
  ...
  data() {
    return {
      headers: [
        ...
        { title: 'Stopbits', key: 'stopbits' },
        { title: 'Parity', key: 'parity' },
        { title: 'Inputlen', key: 'inputlen' },
        { title: 'Threshold', key: 'threshold' },
        { title: 'Delimiter', key: 'delimiter' },
  ...
  methods: {
    ...
    submitDelete() {
      this.isLoading = true
      /*this.$inertia.delete(`/devices/${this.deleteId}`, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
